...
def parse_baseline_a(file_path: Path):
...
def force_curve_to_zero(data: np.ndarray) -> np.ndarray:
    ...
    for i in range(1, len(data)):
        ...
        if y0 > 0 and y1 <= 0 and x1 != x0 and y1 != y0:
            ...
            positive_prefix = data[:i]
            return np.vstack([positive_prefix, [x_zero, 0.0]])
...
def main():
    ...
    data_file = base_dir / "BaselineA.txt"

    datasets = parse_baseline_a(data_file)
    ...
    plt.ylabel("Force")
    plt.title("Baseline A: Force-Displacement")
    plt.xlim(left=0)
    plt.ylim(bottom=0)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    fd_out = base_dir / "baselineA_force_displacement.png"
    ...
    plt.title("Baseline A: Energy Ratio vs Time")
    plt.yscale("log")
    plt.grid(True, which="both", alpha=0.3)
    plt.legend()
    plt.tight_layout()
    e_out = base_dir / "baselineA_energy_ratio.png"
